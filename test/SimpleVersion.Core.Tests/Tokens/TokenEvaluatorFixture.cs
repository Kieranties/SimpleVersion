...
namespace SimpleVersion.Core.Tests.Tokens
{
    public class TokenEvaluatorFixture
    {
        ...
        [InlineData("")]
        [InlineData("\t\t  ")]
        public void Process_EmptyTokenString_Throws(string tokenString)
        {
            // Arrange
            ...
            action.Should().Throw<ArgumentOutOfRangeException>()
                .And.ParamName.Should().Be("tokenString");
        }
        ...
        public void Process_InvalidFormat_Throws(string tokenString)
        {
            // Arrange
            var sut = new TokenEvaluator(Enumerable.Empty<ITokenHandler>());
            ...
            action.Should().Throw<Exception>()
                .WithMessage($"Invalid token string: {tokenString}");
        }
