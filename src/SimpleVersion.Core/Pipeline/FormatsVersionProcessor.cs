...
namespace SimpleVersion.Pipeline
{
    ...
    public class FormatsVersionProcessor : IVersionProcessor
    {
        ...
        public void Process(IVersionContext context)
        {
            ...
            context.Result.Formats["semver1"] = _evaluator.Process(new SemverTokenRequest { Version = 1 }, context);
            context.Result.Formats["semver2"] = _evaluator.Process(new SemverTokenRequest(), context);
            context.Result.Version = _evaluator.Process(new VersionTokenRequest(), context);
        }
    }
}
